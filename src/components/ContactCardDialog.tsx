...
import { User, Mail, Phone, MapPin, ExternalLink, Pencil, Check, X, Tag, Receipt, Plus } from "lucide-react";
...
const productCategoryOrder = ["Hardcover Book", "Digital Book", "Course", "Live", "Velocity", "Summit"];
...
function formatShippingAddress(order: any) {
  ...
  if (!street) return "—";
  // Some imported orders already contain the complete formatted address in
  // the address field. Avoid appending city/state/zip a second time.
  if (/\b\d{5}(?:-\d{4})?\b/.test(street)) return street;
  const city = String(order?.city ?? "").trim();
  const stateZip = [order?.state, order?.zip].filter(Boolean).join(" ").trim();
...
const GHL_LOCATION_ID = "0wGj9IjNlSL307m4uy9Q";
...
export default function ContactCardDialog({
  ...
  const { data: ghlContactId } = useQuery({
    queryKey: ["contact-ghl-id", email],
    queryFn: async () => {
  ...
  const { data: transactionsData } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("transactions")
        .select("*, transaction_line_items(category)")
  ...
  // Match the per-line-item Category tags shown on the Transactions page.
  // Manual transactions have no line items, so they fall back to the
  // transaction-level category.
  const productCategories = Array.from(new Set(
    (transactionsData ?? []).flatMap((transaction: any) => {
      const lineItemCategories = (transaction.transaction_line_items ?? [])
        .map((lineItem: any) => lineItem.category)
        .filter(Boolean);
      return lineItemCategories.length > 0
        ? lineItemCategories
        : [transaction.category].filter(Boolean);
    })
  )).sort((a, b) => {
    const aIndex = productCategoryOrder.indexOf(String(a));
    const bIndex = productCategoryOrder.indexOf(String(b));
  ...
  const addBlankBookOrder = () => {
    setEditBookOrders((prev) => [...prev, { _isNew: true, contact_id: contactId, date_ordered: new Date().toISOString(), date_shipped: null, tracking_status: "PRE_TRANSIT", carrier: "—", tracking_number: "—", delivered: false, address: "", city: "", state: "", zip: "" }]);
  };
    ...
    mutationFn: async () => {
      ...
      for (const bo of editBookOrders) {
        if (isNewRecord(bo)) {
          ...
          const { error: bErr } = await supabase.from("book_orders").update({
            date_ordered: bo.date_ordered,
            date_shipped: bo.date_shipped,
            address: bo.address,
            city: bo.city,
            state: bo.state,
            zip: bo.zip,
  ...
  const primaryBookOrder = editBookOrders[0];

  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-[94vw] bg-card border-border text-foreground sm:max-w-[50rem] max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          <div className="flex items-center justify-between pr-10">
            <DialogTitle className="flex items-center gap-2 text-gold font-body text-lg font-semibold tracking-wide">
              ...
              {ghlContactId && (
                <a
                  href={`https://app.gohighlevel.com/v2/location/${GHL_LOCATION_ID}/contacts/detail/${encodeURIComponent(ghlContactId)}`}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-sm p-1 text-steel transition-colors hover:bg-gold/10 hover:text-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold"
                  aria-label={`Open ${contactName} in HighLevel`}
                  title="Open contact in HighLevel"
                >
        ...
        <div className="grid grid-cols-1 gap-4">
          ...
          <div className="space-y-2">
            ...
            {primaryBookOrder && (
              <div className="flex items-start gap-2 text-sm">
                <MapPin className="mt-1 w-3.5 h-3.5 text-steel shrink-0" />
                {editing ? (
                  <div className="grid min-w-0 flex-1 grid-cols-[minmax(0,1fr)_70px_70px] gap-1.5">
                    <Input
                      value={primaryBookOrder.address ?? ""}
                      onChange={(e) => updateRecord(setEditBookOrders, 0, "address", e.target.value)}
                      placeholder="Street address"
                      className="col-span-3 h-7 text-xs bg-muted border-border text-foreground"
                    />
                    <Input
                      value={primaryBookOrder.city ?? ""}
                      onChange={(e) => updateRecord(setEditBookOrders, 0, "city", e.target.value)}
                      placeholder="City"
                      className="h-7 text-xs bg-muted border-border text-foreground"
                    />
                    <Input
                      value={primaryBookOrder.state ?? ""}
                      onChange={(e) => updateRecord(setEditBookOrders, 0, "state", e.target.value)}
                      placeholder="State"
                      className="h-7 text-xs bg-muted border-border text-foreground"
                    />
                    <Input
                      value={primaryBookOrder.zip ?? ""}
                      onChange={(e) => updateRecord(setEditBookOrders, 0, "zip", e.target.value)}
                      placeholder="ZIP"
                      className="h-7 text-xs bg-muted border-border text-foreground"
                    />
                  </div>
                ) : (
                  <span className="text-foreground leading-relaxed">{formatShippingAddress(primaryBookOrder)}</span>
                )}
              </div>
            )}
          </div>
          ...
          <div className="pt-2 border-t border-border/50">
            ...
            <div className="flex gap-1.5 flex-wrap">
              {productCategories.length > 0 ? productCategories.map((p) => (
          ...
          {/* Product tabs */}
          <Tabs defaultValue="book" className="min-w-0 pt-2 border-t border-border/50">
            <TabsList className="bg-muted/50 h-8">
              {allTabs.map((t) => {
                const hasData =
                  (t.key === "book" && (bookOrdersData?.length ?? 0) > 0) ||
                  (t.key === "workshop" && (workshopData?.length ?? 0) > 0) ||
                  (t.key === "live" && (liveData?.length ?? 0) > 0) ||
                  (t.key === "velocity" && ((velocityMemberData?.length ?? 0) > 0 || (velocitySalesData?.length ?? 0) > 0)) ||
                  (t.key === "summit" && (summitData?.length ?? 0) > 0);
                return (
                  <TabsTrigger
                    key={t.key}
                    value={t.key}
              ...
              {editBookOrders.map((bo: any, i: number) => (
                <div key={bo.id ?? `new-book-${i}`} className="space-y-2">
                  ...
                  <div className="grid grid-cols-[max-content_max-content] gap-x-12 gap-y-2">
                    ...
                    <EditableFieldRow label="Date Shipped" value={bo.date_shipped?.split("T")[0]} type="date" editing={editing}
                      onChange={(v) => updateRecord(setEditBookOrders, i, "date_shipped", v)} />
                  </div>
                </div>
              ))}

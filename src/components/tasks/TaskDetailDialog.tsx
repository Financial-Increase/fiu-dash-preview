...
import { CalendarDays, Check, ChevronDown, Link2, Pencil, Send, Share2, Trash2, User, Users } from "lucide-react";
...
import { assigneeNeedsOwnerReview, submitTaskForOwnerReview } from "./taskReview";
...
export default function TaskDetailDialog({
  ...
  const submitForReview = useMutation({
    mutationFn: async () => {
      ...
      return submitTaskForOwnerReview(task, currentMember);
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["tasks"] });
      queryClient.setQueryData<TaskComment[]>(["task-comments", task?.id], (current = []) =>
        current.some((comment) => comment.id === result.comment.id)
          ? current
          : [result.comment, ...current]
      );
      if (result?.emailWarning) {
        toast.warning("Submitted for review, but the owner email could not be sent.");
      } else {
  ...
  const deleteTask = useMutation({
    mutationFn: async () => {
      ...
      const { data: attachments, error: attachmentsError } = await supabase
        ...
        .select("storage_path")
        .eq("task_id", task.id);
      const attachmentTableIsMissing =
        attachmentsError?.code === "PGRST205" ||
        attachmentsError?.message.includes("task_attachments");
      if (attachmentsError && !attachmentTableIsMissing) throw attachmentsError;
  ...
  const needsOwnerReview = assigneeNeedsOwnerReview(task, currentMember?.id ?? null, isAdmin);
  const availableStatuses = needsOwnerReview
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={handleDialogOpenChange}>
        <DialogContent className="max-h-[90vh] w-[calc(100vw-2rem)] min-w-0 overflow-x-hidden overflow-y-auto [overflow-wrap:anywhere] sm:max-w-5xl">
        <DialogHeader>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {canManage && task.status !== "Done" && needsOwnerReview && (
            <Button
              size="sm"
              className="h-7 gap-1 text-xs"
              disabled={submitForReview.isPending || task.status === "Waiting For"}
              onClick={() => submitForReview.mutate()}
            >
              <Send className="w-3.5 h-3.5" />
              {task.status === "Waiting For" ? "Awaiting Review" : "Ready for Review"}
            </Button>
          )}
          {canManage && task.status !== "Done" && !needsOwnerReview && (
            <Button
        ...
        {canManage && isEditingDescription ? (
          ...
          <div
            className={cn(
              "group relative min-h-16 rounded-sm px-2 py-1 text-sm leading-relaxed",
              canManage && "cursor-text hover:bg-muted/30",
            )}
            onClick={() => {
              if (canManage) setIsEditingDescription(true);
            }}
          >
            {description ? (
              <p className="whitespace-pre-wrap text-foreground">{renderWithLinks(description)}</p>
            ) : canManage ? (
              <p className="text-steel">Add a description…</p>
            ) : null}
            {canManage ? (
              <Pencil className="absolute right-2 top-2 h-3.5 w-3.5 text-steel opacity-0 transition-opacity group-hover:opacity-100" />
            ) : null}
          </div>
        ) : null}

        {canManage && (isDirty || isEditingDescription) && (
          <div className="flex items-center justify-end gap-2 -mt-1">
            ...
            <Button
              ...
              disabled={!isDirty || saveTaskDraft.isPending}
            >
              Save
            </Button>
          </div>

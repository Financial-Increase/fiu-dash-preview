...
export function compareTasksForStatus(status: TaskStatus, a: TaskRow, b: TaskRow) {
  if (status === "Done") {
    const aCompletedAt = a.completed_at ? Date.parse(a.completed_at) : 0;
    const bCompletedAt = b.completed_at ? Date.parse(b.completed_at) : 0;
    return bCompletedAt - aCompletedAt || b.created_at.localeCompare(a.created_at);
  }

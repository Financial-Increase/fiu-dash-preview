...
import { CalendarDays, ChevronDown, Paperclip, Trash2, Upload, User, Users } from "lucide-react";
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (pendingAttachments.length > 0) {
        try {
          await uploadTaskAttachments(taskId, currentMemberId, pendingAttachments);
        } catch (error) {
          attachmentWarning = error instanceof Error ? error.message : "The attachments could not be uploaded";
        }
      }
      return { attachmentWarning, taskId };
    },
    onSuccess: (result) => {
      queryClient.invalidateQueries({ queryKey: ["tasks"] });
      queryClient.invalidateQueries({ queryKey: ["task-attachments", result.taskId] });
      if (result?.attachmentWarning) {
        toast.warning(`${isEdit ? "Task updated" : "Task created"}, but an attachment failed: ${result.attachmentWarning}`);
      } else {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2 border-t border-border/50 pt-3">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              {pendingAttachments.length > 0 ? (
                <span className="text-xs text-steel">{pendingAttachments.length}</span>
              ) : null}
            </div>
            <input
              ...
              onChange={(event) => {
                ...
                const oversized = selected.find((file) => file.size > MAX_TASK_ATTACHMENT_SIZE);
                if (oversized) {
                  toast.error(`${oversized.name} is larger than the 10 MB limit`);
                } else {
          ...
          {pendingAttachments.length === 0 ? (
            <p className="text-xs text-steel">Add files now and they’ll upload when the task is saved.</p>
          ) : (
            <div className="divide-y divide-border/50 rounded-md border border-border/60">
              {pendingAttachments.map((file, index) => (
                <div key={`${file.name}-${file.size}-${file.lastModified}-${index}`} className="flex items-center gap-2 px-3 py-2">
                  <Paperclip className="h-3.5 w-3.5 flex-shrink-0 text-steel" />
                  <span className="min-w-0 flex-1 truncate text-sm">{file.name}</span>
                  <span className="text-[11px] text-steel">{formatTaskAttachmentSize(file.size)}</span>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"

...
import { STATUS_COLORS, STATUS_DESCRIPTIONS, TASK_STATUSES, TaskRow, TaskStatus, TeamMemberLite, compareTasksForStatus, initials } from "./types";
...
const GRID_COLS = "grid-cols-[24px_28px_minmax(0,1fr)] md:grid-cols-[24px_28px_minmax(0,1fr)_130px_100px_110px_130px_90px_110px]";
...
export default function TaskListView({
  ...
  for (const s of TASK_STATUSES) {
    grouped.get(s)!.sort((a, b) => compareTasksForStatus(s, a, b));
  }
  ...
  const handleDragEnd = (result: DropResult) => {
    ...
    const movingTask = displayedTasks.find((task) => task.id === draggableId);
    const requiresOwnerReview =
      destStatus === "Done" &&
      movingTask &&
      !isAdmin &&
      currentMemberId === movingTask.assignee_id &&
      movingTask.owner_id !== movingTask.assignee_id;
    if (requiresOwnerReview) {
      onQuickDone(movingTask);
      return;
    }
  ...
  return (
    <div className="rounded-lg border border-border overflow-hidden">
      <div className={cn("hidden items-center bg-muted/30 border-b border-border md:grid", GRID_COLS)}>
        <div className="h-8" />
        <div className="h-8" />
                            ...
                            {(dragProvided, dragSnapshot) => (
                              ...
                              >
                                ...
                                <div className="text-sm font-medium text-foreground min-w-0 self-stretch">
                                  <button
                                    onClick={() => onOpen(task)}
                                    className="w-full h-full flex flex-col items-start justify-center gap-1.5 text-left py-3 pr-3 hover:underline decoration-steel/40 underline-offset-2 min-w-0 md:flex-row md:items-center md:gap-2 md:py-1.5"
                                  >
                                    <span className={cn("w-full break-words leading-snug md:truncate", task.status === "Done" && "line-through text-steel")}>{task.title}</span>
                                    {task.status === "Done" && task.completed_at && (
                                      <span className="hidden flex-shrink-0 text-[11px] font-normal text-steel md:inline">
                                        (done {format(new Date(task.completed_at), "M/d/yy")})
                                      </span>
                                    )}
                                    ...
                                    <span className="flex w-full flex-wrap items-center gap-x-3 gap-y-1 text-[11px] font-normal text-steel md:hidden">
                                      {task.due_date && (
                                ...
                                <div className="hidden py-1.5 pr-2 md:block">
                                  <Select
                                    value={task.status}
                                ...
                                <div className="hidden py-1.5 pr-3 text-xs text-steel truncate md:block">{task.owner?.name ?? "—"}</div>
                              </div>
                            )}
                          </Draggable>
                        );

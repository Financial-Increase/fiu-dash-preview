...
const columns: { key: keyof ShipmentRecord; label: string; type?: ColumnType; options?: string[]; minWidth: number; editable?: boolean }[] = [
  { key: "date_ordered", label: "Date Ordered", type: "date", minWidth: 150, editable: false },
...
function formatToPacific(isoString: string, withTime = false): string {
  if (!isoString) return "";
  try {
          ...
          if (col.editable === false) {
            return (
              <span className="text-foreground">
                {col.key === "date_ordered" ? formatToPacific(row.date_ordered, true) : (raw ?? "—")}
              </span>
            );
          }
          return (

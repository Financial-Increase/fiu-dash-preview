...
import { assigneeNeedsOwnerReview, submitTaskForOwnerReview } from "@/components/tasks/taskReview";
...
export default function TasksPage() {
  ...
  const submitForReview = useMutation({
    mutationFn: async ({ task, position }: { task: TaskRow; position?: number }) => {
      if (!currentMember) throw new Error("No team member profile linked to your login");
      return submitTaskForOwnerReview(task, currentMember, position);
    },
    onSuccess: (result) => {
    ...
    <div className="space-y-5 sm:space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="flex w-full min-w-0 items-center gap-2 sm:contents">
          <Button size="sm" className="h-9 flex-none gap-1.5 sm:h-8" onClick={openCreate}>
          ...
          <div className="relative min-w-0 flex-1 sm:w-56 sm:flex-none">
            ...
            <Input
              ...
              className="h-9 pl-8 text-xs bg-background sm:h-8"
            />
          </div>
        </div>
        ...
        <div className="flex w-full min-w-0 items-center gap-2 sm:contents">
          <Select
            ...
            disabled={!isAdmin}
          >
            <SelectTrigger className="h-9 min-w-0 flex-1 text-xs bg-background disabled:cursor-not-allowed disabled:opacity-50 sm:h-8 sm:w-[160px] sm:flex-none">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
          ...
          <Popover open={tagFilterOpen} onOpenChange={setTagFilterOpen}>
            <PopoverTrigger asChild>
              <Button
                ...
                variant="outline"
                size="sm"
                className="h-9 min-w-0 flex-1 justify-between text-xs font-normal bg-background sm:h-8 sm:w-[150px] sm:flex-none"
              >
        ...
        <div className="flex w-full items-center rounded-lg border border-border overflow-hidden sm:w-auto">
          <button
            onClick={() => setView("list")}
            className={cn(
              "h-9 flex-1 px-2.5 flex items-center justify-center gap-1.5 text-xs font-medium transition-colors sm:h-8 sm:flex-none",
          ...
          <button
            ...
            className={cn(
              "h-9 flex-1 px-2.5 flex items-center justify-center gap-1.5 text-xs font-medium transition-colors border-l border-border sm:h-8 sm:flex-none",
      ...
      {tasksError ? (
        ...
        <TaskListView
          ...
          onOpen={openDetail}
          onQuickDone={(task) => {
            if (task.status !== "Done" && assigneeNeedsOwnerReview(task, currentMember?.id ?? null, isAdmin)) {
              submitForReview.mutate({ task });
              return;
            }
        ...
        <TaskKanbanBoard
          ...
          onMove={(id, fields) => {
            ...
            if (
              task &&
              fields.status === "Done" &&
              assigneeNeedsOwnerReview(task, currentMember?.id ?? null, isAdmin)
            ) {
              submitForReview.mutate({ task, position: fields.position });
              return;
            }
